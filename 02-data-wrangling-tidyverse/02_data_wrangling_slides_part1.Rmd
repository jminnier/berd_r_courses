...
- `!` in front negates the statement, as in `!is.na(age)` or `!(grade%in%c("9th","10th"))`
]
...
1. Import `demo_data.csv` in the `data` folder if you haven't already done so.
1. Create a variable called `grade_num` that has the numeric grade number.
...
# Advanced `mutate_*()` and `case_when()` examples
